...
{      
   ...
   console.log(usuario);
    ...
    .then
    (
        async (listaComentarios)=>
        {
            const titulosDePeliculas = await Promise.all(listaComentarios.map(comentario => peliculas.find({idPelicula: comentario.idPelicula}).then(peliculas => peliculas[0].nombre)))
